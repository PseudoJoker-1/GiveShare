...
def get_geolocation_from_ip(ip):
    try:
        ...
        data = response.json()
        print(f"Geolocation data for IP {ip}: {data}")  # Debugging line
        country_code = data.get('country')
        country_name = COUNTRY_CODE_TO_NAME.get(country_code, country_code)  # Default to country code if name not found
        return {
            'city': data.get('city'),
...
def get_timezone_from_ip(ip):
    try:
        ...
        data = response.json()
        print(f"Geolocation data for IP {ip}: {data}")  # Debugging line to check API response
        if 'timezone' in data:
            print(f"Detected timezone: {data['timezone']}")  # Debugging line to verify timezone
